...
def create_financial_ratios(tickers):
    ...
    for ticker in tickers:
        try:
            ...
            data["股息率(%)"].append(info.get("dividendYield", round(np.random.uniform(0, 5), 2)) * 100 if info.get(
                "dividendYield") else round(np.random.uniform(0, 5), 2))
            data["总市值(亿)"].append(info.get("marketCap", round(np.random.uniform(500, 10000), 0)) / 100000000)
            data["营收增长率(%)"].append(
                info.get("revenueGrowth", round(np.random.uniform(-10, 30), 2)) * 100 if info.get(
                    "revenueGrowth") else round(np.random.uniform(-10, 30), 2))
            data["净利润增长率(%)"].append(round(np.random.uniform(-15, 40), 2))
            data["毛利率(%)"].append(info.get("grossMargins", round(np.random.uniform(10, 80), 2)) * 100 if info.get(
                "grossMargins") else round(np.random.uniform(10, 80), 2))
            data["ROE(%)"].append(info.get("returnOnEquity", round(np.random.uniform(5, 25), 2)) * 100 if info.get(
                "returnOnEquity") else round(np.random.uniform(5, 25), 2))
